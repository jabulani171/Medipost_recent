var db          = require('../db_schema/shippingStation');
...
function _DtGetData(req, loggedInUser, callback){
    ...
    if(strSearch){
		...
		let Filters = [{'Shipping_Station_IP': regex},
					   {'Shipping_Station_ID': regex},
					   {'Label_Printer': regex},
					   {'ActiveWaybill': regex}];

		if(!isNaN(strSearch)){
			Filters.push({'Shipping_Station_ID': Number(strSearch)});
		}
	...
	db.countDocuments({}, function (err, c) {
		...
		db.countDocuments(searchStr, function(err, c) {
			...
			db.find(searchStr, 'Shipping_Station_IP Shipping_Station_ID Label_Printer ActiveWaybill',
				...
				while(x < results.length){
					let mData = {
						Shipping_Station_IP: results[x].Shipping_Station_IP,
						Shipping_Station_ID: results[x].Shipping_Station_ID,
						Label_Printer: results[x].Label_Printer,
						ActiveWaybill: results[x].ActiveWaybill,			
						StatReprint: req.session.UserPriv.indexOf('ManageShippingStation') >= 0? true: false
					}
